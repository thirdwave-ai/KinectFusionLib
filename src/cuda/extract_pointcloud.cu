...
__global__ void extract_points_kernel(
    ...
    PtrStep<float3> vertices,
    PtrStep<float3> normals,
    PtrStep<uchar3> color,
    int* point_num) {
...
PointCloud extract_points(const VolumeData& volume, const int buffer_size) {
  ...
  dim3 threads(32, 32);
  dim3 blocks((volume.volume_size.x + threads.x - 1) / threads.x, (volume.volume_size.y + threads.y - 1) / threads.y);
  ...
  return PointCloud{
      cloud_data.host_vertices, cloud_data.host_normals, cloud_data.host_color, cloud_data.host_point_num};
}
}  // namespace cuda
}  // namespace internal
}  // namespace kinectfusion