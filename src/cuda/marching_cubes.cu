...
//##### HELPERS #####
static __device__ __forceinline__ unsigned int lane_ID() {
  unsigned int ret;
...
__device__ __forceinline__ float read_tsdf(
    const PtrStep<short2> tsdf_volume, const int3 volume_size, const int x, const int y, const int z, short& weight) {
...
__device__ __forceinline__ int compute_cube_index(
  ...
  if (weight == 0)
    return 0;
  cube_index += static_cast<int>(tsdf_values[2] = read_tsdf(tsdf_volume, volume_size, x + 1, y + 1, z, weight) < 0.f)
                << 2;
  if (weight == 0)
    return 0;
  ...
  if (weight == 0)
    return 0;
  cube_index += static_cast<int>(tsdf_values[5] = read_tsdf(tsdf_volume, volume_size, x + 1, y, z + 1, weight) < 0.f)
                << 5;
  if (weight == 0)
    return 0;
  cube_index +=
      static_cast<int>(tsdf_values[6] = read_tsdf(tsdf_volume, volume_size, x + 1, y + 1, z + 1, weight) < 0.f) << 6;
  if (weight == 0)
    return 0;
  cube_index += static_cast<int>(tsdf_values[7] = read_tsdf(tsdf_volume, volume_size, x, y + 1, z + 1, weight) < 0.f)
                << 7;
  if (weight == 0)
    return 0;
...
__device__ __forceinline__ float3 get_node_coordinates(const int x, const int y, const int z, const float voxel_size) {
  float3 position;
...
__device__ __forceinline__ float3 vertex_interpolate(const float3 p0, const float3 p1, const float f0, const float f1) {
  float t = (0.f - f0) / (f1 - f0 + 1e-15f);
  return make_float3(p0.x + t * (p1.x - p0.x), p0.y + t * (p1.y - p0.y), p0.z + t * (p1.z - p0.z));
}

//##### KERNELS #####
__global__ void get_occupied_voxels_kernel(
    const PtrStep<short2> volume,
    const int3 volume_size,
    PtrStepSz<int> occupied_voxel_indices,
    PtrStepSz<int> number_vertices,
  ...
  const auto flattened_tid = threadIdx.z * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
...
__global__ void generate_triangles_kernel(
    ...
    const PtrStepSz<int> occupied_voxels,
    const PtrStepSz<int> vertex_offsets,
    const PtrStep<int> number_vertices_table,
    const PtrStep<int> triangle_table,
  ...
  for (int i = 0; i < n_vertices; i += 3) {
    ...
    triangle_buffer.ptr(0)[index + 0] =
        make_float3(vertex_list[v1][threadIdx.x].x, vertex_list[v1][threadIdx.x].y, vertex_list[v1][threadIdx.x].z);
    triangle_buffer.ptr(0)[index + 1] =
        make_float3(vertex_list[v2][threadIdx.x].x, vertex_list[v2][threadIdx.x].y, vertex_list[v2][threadIdx.x].z);
    triangle_buffer.ptr(0)[index + 2] =
        make_float3(vertex_list[v3][threadIdx.x].x, vertex_list[v3][threadIdx.x].y, vertex_list[v3][threadIdx.x].z);
  }
}

__global__ void get_color_values_kernel(
    const PtrStep<uchar3> color_volume,
    const int3 volume_size,
    const float voxel_scale,
    const PtrStep<float3> vertices,
  ...
  uchar3 color_value = color_volume.ptr(location_in_grid.z * volume_size.y + location_in_grid.y)[location_in_grid.x];
...
SurfaceMesh marching_cubes(const VolumeData& volume, const int triangles_buffer_size) {
  ...
  dim3 threads(32, 32);
  dim3 blocks(
      static_cast<unsigned>(std::ceil(volume.volume_size.x / threads.x)),
      static_cast<unsigned>(std::ceil(volume.volume_size.y / threads.y)));

  get_occupied_voxels_kernel<<<blocks, threads>>>(
      volume.tsdf_volume,
      volume.volume_size,
  ...
  cudaMemcpy(
      &last_element, mesh_data.number_vertices.ptr<int>(0) + active_voxels - 1, sizeof(int), cudaMemcpyDeviceToHost);
  cudaMemcpy(
      &last_scan_element,
      mesh_data.vertex_offsets.ptr<int>(0) + active_voxels - 1,
      sizeof(int),
  ...
  generate_triangles_kernel<<<grid, block>>>(
      ...
      volume.volume_size,
      volume.voxel_scale,
      mesh_data.occupied_voxel_ids,
      mesh_data.vertex_offsets,
      number_vertices_table,
      triangle_table,
  ...
  get_color_values_kernel<<<n_blocks, 1024>>>(
      volume.color_volume, volume.volume_size, volume.voxel_scale, triangles_output, vertex_colors);
