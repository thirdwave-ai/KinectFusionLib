...
__global__ void estimate_kernel(
    ...
    const PtrStep<float3> vertex_map_previous,
    const PtrStep<float3> normal_map_previous,
  ...
  if (x < cols && y < rows) {
    ...
    if (!isnan(normal_current.x())) {
      ...
      Matf31da vertex_current_camera = rotation_previous_inv * (vertex_current_global - translation_previous);
...
__global__ void reduction_kernel(PtrStep<double> global_buffer, const int length, PtrStep<double> output) {
  double sum = 0.0;
...
void estimate_step(
    ...
    const cv::cuda::GpuMat& normal_map_current,
    const Eigen::Matrix3f& rotation_previous_inv,
    const Matf31da& translation_previous,
    const CameraParameters& cam_params,
    const cv::cuda::GpuMat& vertex_map_previous,
    const cv::cuda::GpuMat& normal_map_previous,
