...
__device__ __forceinline__ float interpolate_trilinearly(
    const Vec3fda& point, const PtrStepSz<short2>& volume, const int3& volume_size, const float voxel_scale) {
  ...
  return static_cast<float>(volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y())[point_in_grid.x()].x) *
             DIVSHORTMAX * (1 - a) * (1 - b) * (1 - c) +
         static_cast<float>(
             volume.ptr((point_in_grid.z() + 1) * volume_size.y + point_in_grid.y())[point_in_grid.x()].x) *
             DIVSHORTMAX * (1 - a) * (1 - b) * c +
         static_cast<float>(
             volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x()].x) *
             DIVSHORTMAX * (1 - a) * b * (1 - c) +
         static_cast<float>(
             volume.ptr((point_in_grid.z() + 1) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x()].x) *
             DIVSHORTMAX * (1 - a) * b * c +
         static_cast<float>(
             volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y())[point_in_grid.x() + 1].x) *
             DIVSHORTMAX * a * (1 - b) * (1 - c) +
         static_cast<float>(
             volume.ptr((point_in_grid.z() + 1) * volume_size.y + point_in_grid.y())[point_in_grid.x() + 1].x) *
             DIVSHORTMAX * a * (1 - b) * c +
         static_cast<float>(
             volume.ptr((point_in_grid.z()) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x() + 1].x) *
             DIVSHORTMAX * a * b * (1 - c) +
         static_cast<float>(
             volume.ptr((point_in_grid.z() + 1) * volume_size.y + point_in_grid.y() + 1)[point_in_grid.x() + 1].x) *
             DIVSHORTMAX * a * b * c;
}
...
__global__ void raycast_tsdf_kernel(
    const PtrStepSz<short2> tsdf_volume,
    const PtrStepSz<uchar3> color_volume,
  ...
  const float3 volume_range =
      make_float3(volume_size.x * voxel_scale, volume_size.y * voxel_scale, volume_size.z * voxel_scale);
  ...
  float tsdf =
      static_cast<float>(
          tsdf_volume.ptr(__float2int_rd(grid(2)) * volume_size.y + __float2int_rd(grid(1)))[__float2int_rd(grid(0))]
              .x) *
      DIVSHORTMAX;
  ...
  for (; ray_length < max_search_length; ray_length += truncation_distance * 0.5f) {
    ...
    if (grid.x() < 1 || grid.x() >= volume_size.x - 1 || grid.y() < 1 || grid.y() >= volume_size.y - 1 ||
    ...
    if (previous_tsdf > 0.f && tsdf < 0.f) {  // Zero crossing
      ...
      if (location_in_grid.x() < 1 | location_in_grid.x() >= volume_size.x - 1 || location_in_grid.y() < 1 ||
          location_in_grid.y() >= volume_size.y - 1 || location_in_grid.z() < 1 ||
          location_in_grid.z() >= volume_size.z - 1)
        break;
      ...
      model_color.ptr(y)[x] = color_volume.ptr(
          location_in_grid_int.z() * volume_size.y + location_in_grid_int.y())[location_in_grid_int.x()];
...
void surface_prediction(
  ...
  dim3 threads(32, 32);
  dim3 blocks((model_vertex.cols + threads.x - 1) / threads.x, (model_vertex.rows + threads.y - 1) / threads.y);
