...
__global__ void update_tsdf_kernel(
  ...
  for (int z = 0; z < volume_size.z; ++z) {
    ...
    if (sdf >= -truncation_distance) {
      ...
      tsdf_volume.ptr(z * volume_size.y + y)[x] =
          make_short2(static_cast<short>(new_value), static_cast<short>(new_weight));
      ...
      if (sdf <= truncation_distance / 2 && sdf >= -truncation_distance / 2) {
        ...
        model_color.x = static_cast<uchar>(
            (current_weight * model_color.x + add_weight * image_color.x) / (current_weight + add_weight));
        model_color.y = static_cast<uchar>(
            (current_weight * model_color.y + add_weight * image_color.y) / (current_weight + add_weight));
        model_color.z = static_cast<uchar>(
            (current_weight * model_color.z + add_weight * image_color.z) / (current_weight + add_weight));
      }
    }
  }
}
...
void surface_reconstruction(
    const cv::cuda::GpuMat& depth_image,
  ...
  const dim3 blocks(
      (volume.volume_size.x + threads.x - 1) / threads.x, (volume.volume_size.y + threads.y - 1) / threads.y);
