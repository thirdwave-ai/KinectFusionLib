  ...
  for (int level = pyramid_height - 1; level >= 0; --level) {
    for (int iteration = 0; iteration < iterations[level]; ++iteration) {
      ...
      cuda::estimate_step(
          ...
          frame_data.vertex_pyramid[level],
          frame_data.normal_pyramid[level],
          ...
          distance_threshold,
          sinf(angle_threshold * 3.14159254f / 180.f),
          A,
          b);
      ...
      auto camera_rotation_incremental(
          Eigen::AngleAxisf(gamma, Eigen::Vector3f::UnitZ()) * Eigen::AngleAxisf(beta, Eigen::Vector3f::UnitY()) *
